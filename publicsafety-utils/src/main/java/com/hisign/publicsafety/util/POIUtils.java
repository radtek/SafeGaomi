...
//						cell.setCellValue(org.apache.commons.beanutils.BeanUtils.getProperty(obj,getPropertyName(headers.get(x))));
	...
	public static void handleDataToExcel(HSSFWorkbook workbook, List list, Class clazz, String sheetName,
...
//		return workbook;
	...
	public static void exportToExcel(HttpServletResponse response,String fileName,List objs, Class clazz,
		...
		try {
			String tempName = new String(fileName.getBytes(), Constant.DOWNLOAD_FILENAME_CHARSET);
	...
	/**
	 * 多个表导出到一个excel的不同sheet页
	 * @param response
	 * @param fileName
	 * @param excelDatas
	 * @param pageSize
	 */
	public static void exportToExcel(HttpServletResponse response,String fileName,List<ExcelData> excelDatas, int pageSize){
		OutputStream out = null;
		try {
			String tempName = new String(fileName.getBytes(), Constant.DOWNLOAD_FILENAME_CHARSET);
			response.setHeader("content-disposition", "attachment;filename=" + tempName + ".xls");
			...
			for(ExcelData excelData:excelDatas){
				handleDataToExcel(workbook, excelData.objs, excelData.clazz, excelData.sheetName, pageSize);
			}
			out = response.getOutputStream();
			workbook.write(out);
		} catch (UnsupportedEncodingException e) {
