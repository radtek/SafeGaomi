 ...
 * @version 1.0v
 */
@Service("o2oUserDetailsService")
public class O2oUserDetailServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		User userdetail = new User(users.getAccountName(), users.getPassword(), 
	...
	private Set<GrantedAuthority> obtionGrantedAuthorities(Account account) {
		...
		for (Resources res : menus) {
			// TODO:ZZQ 用户可以访问的资源名称（或者说用户所拥有的权限） 注意：必须"RES_"开头
			// 关联代码：applicationContext-security.xml
			// 关联代码：SecurityMetadataSource#loadResourceDefine
			authSet.add(new SimpleGrantedAuthority("RES_" + res.getResKey()));
		}
		return authSet;
	}
}
